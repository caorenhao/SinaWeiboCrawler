 ...
 *
 */
public class TplIndexSpecial {
	
	private static Log LOGGER = LoggerConfig.getLog(TplIndexSpecial.class);

	/** 模板路径 */
	private static String tplPath = "data/parse_tpls";
	 ...
	 */
	public static void saveTpl(String domain, File tplFile, boolean isList) {
		TplIndexItem item = tplIndex.get(domain);
		if (item == null) {
			item = new TplIndexItem();
			tplIndex.put(domain, item);
		}
	...
	public static void refreshIndex() {
		synchronized (isRefresh) {
			if (isRefresh.booleanValue() == true) {
				return;
			}
	...
	private static void loadTemplate(File domainDir) {
		...
		if (listDir.exists()) {
			...
			for(File file: listFiles) {
				try {
					// 保存模板
					saveTpl(domainDir.getName(), file, true);
				} catch (Exception e) {
		...
		if (articleDir.exists()) {
			...
			for(File file: articleFiles) {
				try {
					saveTpl(domainDir.getName(), file, false);
				} catch (Exception e) {
	 ...
	 * 初始化第一次加载
	 */
	public static void init(String tplPath) {
		TplIndexSpecial.tplPath = tplPath;
		refreshIndex();
	}
